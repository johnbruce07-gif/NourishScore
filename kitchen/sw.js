// NourishKitchen Service Worker — scope: /kitchen/
const CACHE = 'nourishkitchen-v1';
...
self.addEventListener('fetch', e => {
  ...
  if (['api.anthropic.com', 'www.themealdb.com'].includes(url.hostname)) return;
  ...
  e.respondWith(
    caches.open(CACHE).then(async cache => {
      ...
      return new Response('NourishKitchen is offline.', { status: 503 });
    })
  );
});
